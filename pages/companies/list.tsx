    ...
    <Layout page='companies' >
      <Head>
        <title>
          {[
            t('brand.title'),
            t('pages.professionals.meta.title')
          ].join(' - ')}
        </title>
          ...
          <ul role="list" className="divide-y divide-gray-200">
            {
              ...
              professionalsData.map((position: position) => (
                <li key={position.id}>
              ...
              <div className="px-4 py-4 sm:px-6">
                <p className="text-sm font-medium text-indigo-600">
                  {t('pages.professionals.notFound')}
                </p>
              </div>
            }
          </ul>
...
export const getStaticProps = async () => {
  const professionals = await db.collection('professionals').orderBy('created', 'desc').get();
  const professionalsData = professionals.docs.map((entry: any) => ({
    id: entry.id,
    ...entry.data()
  }));
  return {
    props: { professionalsData },
    revalidate: 10
  };
};
